...
def SearchExternsionStrReturnFullName(SearchPath, FileKeyword):
    AllSrcFilesArray = os.listdir(SearchPath)
    for FileList in AllSrcFilesArray:
        # find and match files
        result = FileList.endswith(FileKeyword)
        if result:
            print('Full name of target file :', FileList)
            return FileList
        else:
            print('can\'t find MAP file, please check again')

print('***get a current dir***')
CurrentScriptPath = os.path.abspath(os.path.dirname(__file__))
print(CurrentScriptPath)
print('***last level path***')
CurrentWorkPath = os.path.abspath(os.path.join(CurrentScriptPath, "../"))
print(CurrentWorkPath)
# sys.path.append(CurrentWorkPath + '\\Tools\\IniFileUpdate\\UpdateIniFunc.py')
MAP_FileFullName = ' '
MAP_FileFullName = SearchExternsionStrReturnFullName(CurrentWorkPath, 'ESC_5ASGMWEQ1001AA.map')
print('MAP file full name', MAP_FileFullName)
# ExcelFilePath = CurrentWorkPath + '\\UpdateIniFileTemplate.xlsm'
MapFilePath = CurrentWorkPath + '\\' + MAP_FileFullName
# print('Excel path:', ExcelFilePath)
print('Map file path:', MapFilePath)
# os.chmod(ExcelFilePath, stat.S_IWRITE)
# os.chmod(MapFilePath, stat.S_IWRITE)
# workbook = xlrd.open_workbook(ExcelFilePath, 'rb')
# FirstSheetPage = workbook.sheet_by_index(0)
...
fileHandler = open(MapFilePath, 'rb')
print(fileHandler)
...
fileHandler.close()
workbook.save(CurrentWorkPath + '\\' + 'test.xlsx')  # 保存文件
